...
class ModelConfig:
    ...
    def is_mm_prefix_lm(self) -> bool:
        ...
        if not hasattr(self.hf_config, "model_type"):
            return False
        if self.hf_config.model_type in MM_PREFIX_LM_MODELS:
            return True
        # gemma4 unified (omni) and any model that declares bidirectional
        # attention over multimodal positions via use_bidirectional_attention.
        text_cfg = (
            self.hf_config.get_text_config()
            if hasattr(self.hf_config, "get_text_config")
            else self.hf_config
        )
        return bool(getattr(text_cfg, "use_bidirectional_attention", None))
