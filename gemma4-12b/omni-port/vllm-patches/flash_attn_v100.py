...
class FlashAttnV100Impl(TritonAttentionImpl):
    ...
    def _triton_with_bidi_gate(
        self, layer, query, key, value, kv_cache, attn_metadata,
        output, output_scale, output_block_scale,
    ):
        """Run the Triton impl, applying the Gemma4 vision bidi mask only on
        sliding-window layers. Full-attention layers must stay causal over the
        image span. This fork shares one attn_metadata object across both layer
        types, so we cannot pre-clear per layer; instead, since layers run
        sequentially, temporarily null mm_prefix_range for full-attn layers and
        restore it afterwards. Sliding layers keep it -> Triton applies bidi.
        """
        mpr = getattr(attn_metadata, "mm_prefix_range", None)
        is_full_attn = getattr(self, "sliding_window", None) is None
        if mpr is not None and is_full_attn:
            attn_metadata.mm_prefix_range = None
            try:
                return super().forward(
                    layer, query, key, value, kv_cache, attn_metadata,
                    output, output_scale, output_block_scale)
            finally:
                attn_metadata.mm_prefix_range = mpr
        return super().forward(
            layer, query, key, value, kv_cache, attn_metadata,
            output, output_scale, output_block_scale)
    ...
    def forward(
        ...
        if not self._supports_flash_v100_path():
            ...
            return self._triton_with_bidi_gate(
                layer, query, key, value, kv_cache, attn_metadata,
                output, output_scale, output_block_scale)

        # Gemma4 bidirectional vision (use_bidirectional_attention='vision'): the
        # image span attends bidirectionally within SLIDING layers (driven by
        # attn_metadata.mm_prefix_range). flash_attn_v100 kernels are causal-only and
        # ignore that field, so route calls that carry a real image span to the Triton
        # impl, which honors mm_prefix_range. The gate helper keeps full-attn layers
        # causal. Text-only requests have empty ranges -> stay on the fast flash path.
        _mpr = getattr(attn_metadata, "mm_prefix_range", None)
        if _mpr and any(_mpr.values()):
            return self._triton_with_bidi_gate(
                layer, query, key, value, kv_cache, attn_metadata,
                output, output_scale, output_block_scale)
