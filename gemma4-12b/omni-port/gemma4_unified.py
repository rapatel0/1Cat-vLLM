...
class Gemma4UnifiedProcessingInfo(Gemma4ProcessingInfo):
    ...
    def get_mm_max_tokens_per_item(
        ...
        # NOTE: the upstream soft-token override used self.ctx.get_merged_mm_kwargs(),
        # which the deployed vLLM 1.1.0 InputProcessingContext does not provide. The
        # override only let an operator shrink soft-tokens-per-image; drop it and use
        # the config default (num_soft_tokens) directly.
