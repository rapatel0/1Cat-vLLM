...
try:  # newer upstream MoE-only names (the dense 31B never instantiates the MoE/router)
...
class Gemma4MLP(nn.Module):
    def __init__(
        ...
        # gelu_pytorch_tanh (Gemma's GeGLU) isn't in this fork's get_act_and_mul_fn
        # registry; use GeluAndMul directly like gemma3.py does.
...
class Gemma4Model(nn.Module, EagleModelMixin):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        if num_experts > 0 and fused_moe_make_expert_params_mapping is not None:
        ...
        else:  # dense (no experts): empty mapping, all weights load normally
            dot_suffix_expert_params_mapping = []
