...
def _merged_kda_state_shape(
    tp_size: int, num_heads: int, head_dim: int, conv_size: int, num_spec: int
) -> tuple[tuple[int, int], tuple[int, int, int]]:
    q, k, v, recurrent = MambaStateShapeCalculator.kda_state_shape(
        tp_size, num_heads, head_dim, conv_kernel_size=conv_size, num_spec=num_spec
    )
    # This implementation stores the three convolutions in one GDN cache.
    if is_conv_state_dim_first():
        conv = (q[0] + k[0] + v[0], q[1])
    else:
        conv = (q[0], q[1] + k[1] + v[1])
...
def _get_kda_state_shape_for_config(
    ...
    shapes = _merged_kda_state_shape(
...
class BailingMoeV3KimiDeltaAttention(PluggableLayer, MambaBase):
    ...
    def get_state_dtype(self) -> tuple[torch.dtype, torch.dtype]:
        ...
        conv, _, _, recurrent = MambaStateDtypeCalculator.kda_state_dtype(
            self.model_config.dtype, self.cache_config.mamba_cache_dtype
        )
        return conv, recurrent

    def get_state_shape(self) -> tuple[tuple[int, int], tuple[int, int, int]]:
        return _merged_kda_state_shape(
            self.tp_size,
            self.num_heads,
            self.head_dim,
    ...
    def __init__(
        ...
        if self.num_heads % self.tp_size:
            raise ValueError("KDA attention heads must divide tensor parallel size")
...
class BailingMoeV3Gate(nn.Module):
    def __init__(self, config: PretrainedConfig, prefix: str = "gate") -> None:
        super().__init__()
        self.expert_bias = (
