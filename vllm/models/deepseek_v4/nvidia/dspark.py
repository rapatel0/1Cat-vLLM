...
try:
    ...
    triton_sparse_mla_head_block_size = None
    matmul_sparse_mla_attention_with_sink = None
...
class DeepSeekV4DSparkLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attn.scale = self.attn.softmax_scale
        self.attn._einsum_recipe = self.attn.mla_attn._einsum_recipe
        self.attn._tma_aligned_scales = self.attn.mla_attn._tma_aligned_scales
        ...
        self.materialized_head_block_size = (
            triton_sparse_mla_head_block_size() or 1
            if triton_sparse_mla_head_block_size is not None
            else 1
        )
    ...
    def _dspark_attention(
        ...
        if self.use_triton_attention:
            ...
            if (
                self.use_materialized_attention
                and matmul_sparse_mla_attention_with_sink is not None
            ):
                q_tokens = q.reshape(
                    batch_size * block_size,
                    self.attn.n_local_heads,
    ...
    def _o_proj(self, o: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_sm70_reference_kernels:
            ...
            return self._wo_b_output(z.flatten(1))
...
class DeepSeekV4DSpark(nn.Module):
    ...
    def _forward_impl(
        ...
        x = mhc_post(x, residual, post_mix, res_mix)
        hc_head = (
            hc_head_torch
            if self.use_sm70_reference_kernels
    ...
    def apply_dspark_markov_bias(
        ...
        if self.use_confidence_scheduling:
            self._dspark_markov_embeds[step_idx] = markov_embed
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for original_name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                dspark_weight_loader = getattr(param, "weight_loader", None)
                if dspark_weight_loader is None:
                    default_weight_loader(param, loaded_weight)
                else:
                    dspark_weight_loader(param, loaded_weight)
                loaded_params.add(name)
