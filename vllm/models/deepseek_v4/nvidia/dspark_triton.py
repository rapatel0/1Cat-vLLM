...
def _dspark_qkv_postprocess_kernel(
    ...
    q_norm = (q * rrms).to(q_ptr.dtype.element_ty).to(tl.float32)
    ...
    even = (even * rrms).to(q_ptr.dtype.element_ty).to(tl.float32)
    odd = (odd * rrms).to(q_ptr.dtype.element_ty).to(tl.float32)
    cos = tl.load(
...
def dspark_qkv_postprocess(
    ...
    rounded back to the input dtype, then RoPE is applied on the tail dimensions.
    """
    if q.dim() != 3:
    ...
    if q.dtype != kv.dtype or q.dtype not in (torch.float16, torch.bfloat16):
        raise ValueError("DSpark Triton q/kv postprocess requires FP16 or BF16")
...
def dspark_inv_rope_bf16_layout(
    ...
    *,
    ...
    """Inverse-RoPE attention output into grouped low-precision WO_A layout.
    ...
    if o.dtype not in (torch.float16, torch.bfloat16):
        raise ValueError("DSpark inverse-RoPE layout requires FP16 or BF16")
...
def _dspark_context_kv_store_kernel(
    ...
    norm = (x * rrms * weight).to(kv_ptr.dtype.element_ty).to(tl.float32)
    ...
    even = (even_x * rrms * even_w).to(kv_ptr.dtype.element_ty).to(tl.float32)
    odd = (odd_x * rrms * odd_w).to(kv_ptr.dtype.element_ty).to(tl.float32)
    cos = tl.load(
        cos_sin_ptr + pos * rope_dim + pair_idx,
        mask=rope_mask,
        other=0.0,
...
def dspark_context_kv_store(
    ...
    if kv.dtype != cache.dtype or kv.dtype not in (torch.float16, torch.bfloat16):
        raise ValueError("DSpark context KV store requires FP16 or BF16 kv/cache")
...
def _dspark_attention_kernel(
    ...
    QK^T and P@V use tl.dot in the query/KV storage dtype.
    """
    ...
    for start in range(0, total_kv, BLOCK_N):
        ...
        acc = acc * alpha[:, None] + tl.dot(p.to(main_kv_ptr.dtype.element_ty), kv)
        m_i = m_new
...
def dspark_triton_attention(
    ...
    block_m = 32
    # A 32x32x512 tile requires 128 KiB of shared memory, exceeding the
    # 96 KiB limit on SM70. Keep the wider query tile, which amortizes each
    # shared KV read across more rows, and narrow the KV tile on wide heads.
    block_n = 16 if block_d >= 512 else 32
...
def _dspark_markov_probs_blocks_kernel(
    ...
    # ``z == z`` excludes NaNs without relying on an architecture-specific
    # isfinite lowering. A fully masked/non-finite row can occur after sampling
    # filters; it must never turn the ``vocab_size`` sentinel into an embedding
    # index on the next Markov step.
    active_mask = v_mask & (z == z) & (z != -float("inf"))
...
def _dspark_markov_probs_reduce_kernel(
    ...
    row_valid = (row_max == row_max) & (row_max != -float("inf"))
    safe_row_max = tl.where(row_valid, row_max, 0.0)
    row_z = tl.sum(
        tl.where(mask & row_valid, bsum * tl.exp(bmax - safe_row_max), 0.0),
        axis=0,
    )
    row_invz = tl.where(row_valid & (row_z > 0.0), 1.0 / row_z, 0.0)
    tl.store(row_max_ptr + batch_pid, safe_row_max)
    tl.store(row_invz_ptr + batch_pid, row_invz)
    ...
    token = tl.where(row_valid & (token >= 0) & (token < vocab_size), token, 0)
...
def _dspark_markov_probs_normalize_kernel(
    ...
    z_valid = v_mask & (z == z) & (z != -float("inf"))
    probs = tl.where(z_valid, tl.exp(z - row_max) * inv_z, 0.0)
