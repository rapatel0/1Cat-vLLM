...
def _sample_from_logits(
    ...
    q = _empty_exponential_noise_like(probs, use_fp64_gumbel)
    if len(generators) != num_tokens:
        q.exponential_()
    for i, generator in generators.items():
        q[i].exponential_(generator=generator)
    next_token_ids = _sample_with_exponential_noise(probs.clone(), q)
    if not sampling_metadata.all_random:
...
def sample_dspark_markov_block(
    ...
    *,
    ...
    if valid_vocab_size is None:
        valid_vocab_size = base_logits.shape[-1]
    if not 0 < valid_vocab_size <= base_logits.shape[-1]:
        raise ValueError(
            "valid_vocab_size must be within the logits width: "
            f"valid={valid_vocab_size}, logits={base_logits.shape[-1]}"
        )
    ...
    prev_token_ids = first_prev_token_ids.long().clamp(min=0, max=valid_vocab_size - 1)
    ...
    for step_idx in range(block_size):
        ...
        if valid_vocab_size < step_logits.shape[-1]:
            step_logits[:, valid_vocab_size:].fill_(-torch.inf)
        if return_probs and not sampling_metadata.all_greedy:
        ...
        # Keep an invalid/non-finite sampling row from becoming an out-of-range
        # VocabParallelEmbedding index on the next Markov step. The logits tail
        # is masked above, so this is a fail-closed guard rather than a normal
        # sampling transform.
        next_token_ids.clamp_(min=0, max=valid_vocab_size - 1)
...
def sample_dspark_markov_block_fused(
    ...
    *,
    ...
    if not 0 < valid_vocab_size <= vocab_size:
        raise ValueError(
            "valid_vocab_size must be within the logits width: "
            f"valid={valid_vocab_size}, logits={vocab_size}"
        )
    if first_prev_token_ids.shape[0] < batch_size:
        raise ValueError(
            "first_prev_token_ids must have one entry per request: "
    ...
    prev_token_ids = (
        first_prev_token_ids[:batch_size].long().clamp(min=0, max=valid_vocab_size - 1)
    )
    for step_idx in range(block_size):
        step_logits = apply_markov_bias(
            base_logits[:, step_idx, :], prev_token_ids, step_idx
        ...
        tokens[:, step_idx].clamp_(min=0, max=valid_vocab_size - 1)
