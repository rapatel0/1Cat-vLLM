...
@pytest.mark.parametrize(
    ("offsets", "error"),
    [
        ([0, 9, 16], "query length exceeds"),
        ([1, 8, 15], "must start at zero"),
        ([0, 8, 15], "terminal offset"),
    ],
)
def test_grouped_verify_rejects_malformed_query_partition(
    offsets: list[int],
    error: str,
) -> None:
    _require_sm70_extension()
    ...
    query = torch.zeros((16, 6, 256), dtype=torch.float16, device="cuda")
    key_cache = torch.zeros((4, 16, 1, 256), dtype=torch.int8, device="cuda")
    value_cache = torch.zeros_like(key_cache)
...
@torch.inference_mode()
def test_grouped_verify_accepts_variable_multi_request_edges() -> None:
    _require_sm70_extension()
    flash_attn_v100 = pytest.importorskip("flash_attn_v100")
    ...
    seq_lens = torch.tensor([24, 24], dtype=torch.int32, device="cuda")
    query_start_loc = torch.tensor([0, 1, 9], dtype=torch.int32, device="cuda")
    output = torch.empty_like(query)
    softmax_scale = 1.0 / math.sqrt(head_size)
    ...
    def reference(offsets: list[int]) -> torch.Tensor:
        result = torch.empty_like(output, dtype=torch.float32)
        key_scale_values = key_scales.repeat_interleave(32, dim=-1)
        value_scale_values = value_scales.repeat_interleave(32, dim=-1)
        for request in range(num_requests):
            query_start = offsets[request]
            query_end = offsets[request + 1]
            ...
            keys = (
                key_cache[pages, page_offsets, 0].float()
                * key_scale_values[pages, 0].float()
            ).half()
            values = (
                value_cache[pages, page_offsets, 0].float()
                * value_scale_values[pages, 0].float()
            ).half()
            prefix_len = seq_len - query_len
            for local_query in range(query_len):
    ...
    with torch.cuda.graph(graph):
        run_grouped_verify()
    query_start_loc.copy_(torch.tensor([0, 8, 9], dtype=torch.int32, device="cuda"))
    query.copy_(torch.randn_like(query))
    graph.replay()
    torch.cuda.synchronize()
    torch.testing.assert_close(
