...
def _distributed_dcp_q1_xqa_a2a_worker(env: dict[str, str]) -> None:
    update_environment_variables(env)
    local_rank = int(env["LOCAL_RANK"])
    torch.accelerator.set_device_index(local_rank)
    dist.init_process_group(backend="nccl")
    from flash_attn_v100 import flash_attn_decode_paged_xqa
    from flash_attn_v100.flash_attn_interface import _decode_workspace_cache
    ...
    capture_stream = None
    try:
        batch, heads, head_dim, block_size = 4, 6, 256, 16
        ...
        generator.manual_seed(900 + rank)
        ...
        local_lens = (
            torch.tensor([0, 1, 5, 9], device=device, dtype=torch.int32)
            if rank == 0
            else torch.tensor([1, 0, 4, 7], device=device, dtype=torch.int32)
        )
        block_table = torch.arange(batch, device=device, dtype=torch.int32).view(
            batch, 1
        )
        local_out = torch.empty_like(query)

        capture_stream = torch.cuda.Stream(device=local_rank)
        capture_stream.wait_stream(torch.cuda.current_stream(local_rank))
        with torch.cuda.stream(capture_stream):
            partial_out, partial_lse = flash_attn_decode_paged_xqa(
                query,
                key_cache,
                value_cache,
                block_table,
        ...
        with torch.cuda.graph(graph, stream=capture_stream):
            ...
            actual_out, actual_lse = dcp_a2a_lse_reduce(
                partial_out,
                partial_lse,
                group,
                return_lse=True,
        ...
        gathered_k = [torch.empty_like(key_cache) for _ in range(world_size)]
        gathered_v = [torch.empty_like(value_cache) for _ in range(world_size)]
        gathered_lens = [torch.empty_like(local_lens) for _ in range(world_size)]
        dist.all_gather(gathered_k, key_cache)
        dist.all_gather(gathered_v, value_cache)
        dist.all_gather(gathered_lens, local_lens)
        ...
        for batch_idx in range(batch):
            keys = torch.cat(
                [
                    gathered_k[owner][batch_idx, : int(gathered_lens[owner][batch_idx])]
            ...
            values = torch.cat(
                [
                    gathered_v[owner][batch_idx, : int(gathered_lens[owner][batch_idx])]
                    for owner in range(world_size)
                ],
                dim=0,
            ).float()
            keys = keys.repeat_interleave(heads, dim=1)
            values = values.repeat_interleave(heads, dim=1)
            scores = torch.einsum(
                "hd,nhd->hn",
                query[batch_idx].float(),
                keys,
            ) * (head_dim**-0.5)
            lse = torch.logsumexp(scores, dim=-1)
            probs = torch.softmax(scores, dim=-1)
            output = torch.einsum("hn,nhd->hd", probs, values)
            expected_out.append(output[head_slice])
            expected_lse.append(lse[head_slice])
...
def test_distributed_dcp2_q1_xqa_q_per_kv6_a2a_cuda_graph():
    _distributed_run(
        _distributed_dcp_q1_xqa_a2a_worker,
        world_size=2,
        extra_env={},
    )
